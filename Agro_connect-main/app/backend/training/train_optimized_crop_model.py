"""
High-Performance Crop Recommendation Model Training Pipeline
Trains multiple ML models on local agro_connect datasets and selects the best performer.
"""
...
# Core agricultural features for crop recommendation
...
def load_dataset(csv_path: Path, source_config: Dict[str, Any], humidity_median: float = 80.0) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        if len(cols_to_keep) < 5:  # Need at least 5 core features
            return None
...
def load_all_datasets() -> Tuple[pd.DataFrame, List[str]]:
    """Load and combine all available crop datasets."""
    datasets = []
    sources_used = []
    ...
    print(f'\nCombined dataset: {len(combined)} rows, {combined[LABEL_COLUMN].nunique()} unique crops')
    
    return combined, sources_used


def prepare_training_data(df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.Series, StandardScaler, LabelEncoder]:
    ...
    for label, count in pd.Series(y_encoded).value_counts().sort_index().items():
        print(f'  {encoder.classes_[label]}: {count}')
...
def train_random_forest(X_train: pd.DataFrame, y_train: np.ndarray) -> Tuple[RandomForestClassifier, Dict[str, float]]:
...
def train_gradient_boosting(X_train: pd.DataFrame, y_train: np.ndarray) -> Tuple[GradientBoostingClassifier, Dict[str, float]]:
    """Train GradientBoosting with hyperparameter tuning."""
...
def main():
    ...
    gb_model, gb_info = train_gradient_boosting(X_train, y_train)
    ...
    metadata = {
        ...
        'model_info': best_result['info'],
        'evaluation': {
            'accuracy': float(best_result['eval']['accuracy']),
            'precision': float(best_result['eval']['precision']),
            'recall': float(best_result['eval']['recall']),
            'f1_score': float(best_result['eval']['f1_score']),
            'classification_report': best_result['eval']['classification_report'],
        },
        'all_models_trained': [
            {
                'name': m['name'],
    ...
    summary = {
        ...
        'total_crops': encoder.classes_.tolist(),
        'datasets_used': sources_used,
    }
    
    print(json.dumps(summary, indent=2))
    
    if best_result['eval']['accuracy'] < 0.85:
        raise RuntimeError(f"Model accuracy below acceptable threshold: {best_result['eval']['accuracy']:.4f}")
